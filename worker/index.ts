...
function compactMetrics(result:ReturnType<typeof score>){const raw=result.diagnostic.raw,clarity=result.public.status==='classified'?result.public.classificationClarity:null,lineage=result.public.status==='classified'?result.public.lineage:null;const common={status:raw.status,selection_version:raw.selection_version,model_fingerprint:raw.model_fingerprint,calibration_version:raw.calibration_version,context_taxonomy_version:raw.context_taxonomy_version,response_counts:raw.response_counts,total_items:raw.total_items,classificationClarity:clarity,lineage};if(raw.status==='INSUFFICIENT_SIGNAL')return common;return {...common,primary_similarity:raw.primary_similarity,runner_up_similarity:raw.runner_up_similarity,global_margin:raw.global_margin,basin_depth:raw.basin_depth,nearest_boundary:raw.nearest_boundary,response_amplitude:raw.response_amplitude,information_coverage:raw.information_coverage,boundary_conflict:raw.boundary_conflict,evidence_consistency:raw.evidence_consistency,registered_suite:raw.registered_suite};}
export default {

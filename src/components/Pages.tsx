...
const encodeResponses=(responses:Responses,itemVersions?:Record<string,number>)=>btoa(JSON.stringify(makeExport(responses,itemVersions))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
...
function validResult(value:ScoringResult|undefined, mode:'quick'|'full'='full'):value is ScoringResult{
 try{
  ...
  return Boolean(civilization(p.primary.id)&&civilization(p.runner_up.id)&&d.ranking.length===21&&new Set(d.ranking.map(row=>row.id)).size===21&&civs.every(c=>Number.isFinite(p.scores[c.lineageId])&&d.ranking.some(row=>row.id===c.lineageId&&Number.isFinite(row.rawCosine)&&Number.isFinite(row.zScore)&&Number.isFinite(row.matchScore)))&&p.stability.label in stabilityLabels&&d.raw.prototype_robustness.total===5&&d.raw.context_robustness.scenarios.length===9&&Number.isFinite(d.raw.global_margin));
 }catch{return false;}
}
...
function ShareButton({responses,civilization,itemVersions}:{responses:Responses;civilization:string;itemVersions?:Record<string,number>}){
 ...
 const url=()=>`${window.location.origin}/report?s2=${encodeResponses(responses,itemVersions)}`;

...
import {MODEL_VERSION,MODE_METADATA,responseMode,makeExport,parseImport,wordingVersionsForMode,type Responses} from '../shared/questionnaire';
...
import CivilizationLineageTree from './CivilizationLineageTree';
...
function validResult(value:ScoringResult|undefined, mode:'quick'|'full'='full'):value is ScoringResult{
 try{
  ...
  return Boolean(civilization(p.primary.id)&&civilization(p.runner_up.id)&&d.ranking.length===21&&new Set(d.ranking.map(row=>row.id)).size===21&&civs.every(c=>Number.isFinite(p.scores[c.lineageId])&&d.ranking.some(row=>row.id===c.lineageId&&Number.isFinite(row.rawCosine)&&Number.isFinite(row.zScore)&&Number.isFinite(row.matchScore)))&&clarity?.form===mode&&clarity.primary===d.ranking[0].id&&clarity.runnerUp===p.runner_up.id&&Number.isFinite(clarity.marginRaw)&&['very_close','close','clear','very_clear'].includes(clarity.baseTier)&&['very_close','close','clear','very_clear'].includes(clarity.tier)&&typeof clarity.prototypeStable==='boolean'&&['stable','sensitive'].includes(p.stability.label)&&lineage?.rawAnchor===d.raw.primary&&lineage.lineageResult&&lineage.resolverVersion==='lineage-v1.1'&&d.raw.prototype_robustness.total===5&&d.raw.context_robustness.scenarios.length===9&&Number.isFinite(d.raw.global_margin));
 }catch{return false;}
}
...
export function OfficialText({id,insertions}:{id:number;insertions?:Partial<Record<number,ReactNode>>}){
 ...
 const cleanChapterTitle=(line:string)=>line.replace(/^#+\s*/,'').replace(/^\d{1,2}\s*(?:[｜|]\s*)?/,'').replace(/\*\*/g,'');
 const chapters=useMemo(()=>lines.map((line,i)=>({line:i+1,title:cleanChapterTitle(line)})).filter((_,i)=>/^#{1,3}\s/.test(lines[i])),[lines]);
 ...
 const chapterNumbers=useMemo(()=>new Map(chapterStarts.map((line,index)=>[line,index+1])),[chapterStarts]);
 ...
 const renderMarkdown=(start:number,end:number,key:string)=>{const source=lines.slice(start,end).map(line=>/^#{1,3}\s/.test(line)?line.replace(/^(#{1,3}\s+)\d{1,2}\s*(?:[｜|]\s*)?/,'$1'):line).join('\n');if(!source)return null;const heading=(p:any)=>{const line=start+(p.node?.position?.start.line||1)-1,number=chapterNumbers.get(line);return <h2 id={`chapter-${line+1}`}>{number&&<span className="official-chapter-number">{String(number).padStart(2,'0')}</span>}{p.children}</h2>;};const components={h1:heading,h2:heading,h3:heading};return <Markdown key={key} components={components}>{source}</Markdown>;};
 let start=0;
...
export function Report({attempt,demo=false,onSaved,onEdit,onRestart}:{attempt?:Attempt|null;demo?:boolean;onSaved?:(a:Attempt)=>void;onEdit?:()=>void;onRestart:()=>void}){
 ...
 const itemVersions=responses?wordingVersionsForMode(mode):undefined;
 const responseKey=JSON.stringify(responses),sourceKey=`${shared.shared?'shared':demo?'demo':localAttempt?.attemptId}:${responseKey}`;
 const [loaded,setLoaded]=useState<{key:string;result:ScoringResult}|null>(null),[error,setError]=useState(''),[retry,setRetry]=useState(0),[saveState,setSaveState]=useState(''),[saving,setSaving]=useState(false),[refreshedForLatest,setRefreshedForLatest]=useState(false);
 ...
 useEffect(()=>{
  ...
  async function load(){
   try{let next:ScoringResult;
    if(localAttempt?.saved){const replaced= !validResult(localAttempt.result,mode);next=await requestScore(responses!,abort.signal);if(active){if(replaced)setRefreshedForLatest(true);onSavedRef.current?.({...localAttempt,result:next});}}
...
function ShareButton({responses,civilization,itemVersions}:{responses:Responses;civilization:string;itemVersions?:Record<string,number>}){
 ...
 return <><button ref={triggerRef} className="share-button" onClick={()=>{setCopied(false);setError('');creationAttempted.current=false;setOpen(true)}}><Copy size={15}/>分享我的結果</button>{open&&createPortal(<div className="share-modal-backdrop" onClick={e=>{if(e.target===e.currentTarget)setOpen(false)}}><div ref={dialogRef} className="share-modal" role="dialog" aria-modal="true" aria-labelledby="share-title"><button ref={closeRef} className="share-close" onClick={()=>setOpen(false)} aria-label="關閉分享視窗">×</button><p className="eyebrow">分享你的星際座標</p><h2 id="share-title">我的主要文明是 {civilization}。<br/><em>你的是什麼？</em></h2><p>連結會以短代碼保存你的答案；持有連結的人可查看報告及匯出答案，請只分享給你願意讓對方閱讀的人。</p><div className="share-actions"><button disabled={!shareUrl} onClick={()=>share('facebook')}>Facebook</button><button disabled={!shareUrl} onClick={()=>share('x')}>X</button><button disabled={!shareUrl} onClick={copy}><Copy size={15}/>{creating?'正在建立短連結……':copied?'已複製連結':'複製短連結'}</button></div>{error&&<p role="alert">{error}</p>}</div></div>,document.body)}</>;
}
function ShareInvitation({responses,civilization,itemVersions}:{responses:Responses;civilization:string;itemVersions?:Record<string,number>}){
 const [status,setStatus]=useState(''),[busy,setBusy]=useState(false);
 async function copyInvitation(){
  setBusy(true);setStatus('');
  try{
   const response=await fetch('/api/shares',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({payload:makeExport(responses,itemVersions)})});
   const data=await response.json() as {error?:string;code?:string};
   if(!response.ok||!data.code)throw Error(data.error||'暫時未能建立分享連結。');
   const message=`我的星際種子是 ${civilization}。你的是什麼？\n\n${window.location.origin}/report?share=${data.code}`;
   await navigator.clipboard.writeText(message);
   setStatus('分享文字已複製，現在可以貼給你想喚醒的人。');
  }catch(error){setStatus((error as Error).message||'未能複製分享文字，請允許瀏覽器使用剪貼簿後再試。');}
  finally{setBusy(false);}
 }
 return <section className="share-invitation" aria-labelledby="share-invitation-title">
  <span className="share-invitation-star" aria-hidden="true">✦</span>
  <div><h2 id="share-invitation-title">一盞燈，能點亮整片森林。</h2><p>如果這段文字觸動了你，或許你身邊也有正等待被喚醒的靈魂。</p></div>
  <button className="button gold" onClick={copyInvitation} disabled={busy}><Copy size={17}/>{busy?'正在建立連結……':'複製分享文字'}</button>
  {status&&<p className="share-invitation-status" role="status">{status}</p>}
 </section>;
}
...
function Ranking({result}:{result:ClassifiedResult}){
 const [expanded,setExpanded]=useState(false),rows=result.diagnostic.ranking;
 const renderRows=(items:typeof rows)=>items.map(s=>{const c=civilization(s.id);return <Link className={`rank-row ${s.id===result.public.primary.id?'is-primary':''}`} key={s.id} to={`/civilizations/${c.id}`}><span className="rank-number">{String(s.rank).padStart(2,'0')}</span><Icon id={c.id} size="small"/><span className="rank-name">{c.name}<small>{s.id===result.public.primary.id?'第一名 · 最接近你':s.id===result.public.runner_up.id?'第二名 · 同樣值得認識':c.category}</small></span><div className="rank-score"><span className="score-track"><i style={{width:`${s.matchScore}%`}}/></span><b>契合度 {displayMatch(s.matchScore)}</b></div><ArrowUpRight className="rank-link" size={15}/></Link>;});
 return <section className="report-section" id="structure"><div className="section-heading"><div><p className="eyebrow">01 / 完整排行</p><h2>你的方向，如何落在星海之中。</h2></div><span className="section-note">21 個文明</span></div><p className="analysis-intro">先看看最靠近你的 6 個文明。50 代表中性匹配；低於 50 不代表好壞，只表示這些特質在本次回答中較少出現。</p><div className="ranking-table"><div className="rank-header"><span>排行 / 文明</span><span>契合度 0–100</span></div>{renderRows(rows.slice(0,6))}</div>{!expanded?<button className="expand-button ranking-expand" onClick={()=>setExpanded(true)}>展開其餘 15 個文明 <ChevronDown size={16}/></button>:<><div className="ranking-table ranking-rest">{renderRows(rows.slice(6))}</div><button className="expand-button ranking-expand" onClick={()=>setExpanded(false)}>收起其餘文明 <ChevronDown size={16}/></button></>}<p className="chart-note">契合度由原始 cosine 依測驗版本換算而成；50 是原始 cosine 為 0 的中性匹配。各文明分開比較，分數不需要加總為 100。</p></section>;
}
function Feedback({attempt}:{attempt:Attempt}){const [fit,setFit]=useState(0),[self,setSelf]=useState(''),[comment,setComment]=useState(''),[status,setStatus]=useState(''),[busy,setBusy]=useState(false),[detailsOpen,setDetailsOpen]=useState(false);async function saveFeedback(includeDetails:boolean){setBusy(true);try{const response=await fetch('/api/feedback',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({attemptId:attempt.attemptId,feedbackToken:attempt.feedbackToken,fit,selfLineage:includeDetails&&self?Number(self):null,comment:includeDetails?comment:''})});const data=await response.json() as {error?:string};if(!response.ok)throw Error(data.error||"暫時無法儲存回饋。");setStatus(includeDetails?'補充回饋已儲存。謝謝你讓這份星圖更加清晰。':'符合程度已儲存。');if(!includeDetails)setDetailsOpen(true);}catch(e){setStatus((e as Error).message);}finally{setBusy(false);}}
 const options=<><option value="">不確定</option>{civs.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</>;
 return <section className="feedback-section report-section" id="feedback"><div><p className="eyebrow">讓你的聲音，回到星圖之中</p><h2>這份記憶，<br/>與你有多靠近？</h2><p>先留下整體感受；送出後，再選擇是否補充最符合或最不像自己的地方。</p><Link className="text-button" to="/privacy">匿名資料如何使用 <ArrowUpRight size={16}/></Link></div><div className="feedback-form-wrap"><form onSubmit={e=>{e.preventDefault();void saveFeedback(false)}}><fieldset><legend>結果符合程度</legend><div className="fit-scale">{[1,2,3,4,5,6,7].map(v=><label key={v} className={fit===v?'selected':''}><input required type="radio" name="fit" value={v} checked={fit===v} onChange={()=>setFit(v)}/><span>{v}</span></label>)}</div><div className="scale-labels"><span>很不符合</span><span>非常符合</span></div></fieldset><button className="button gold" type="submit" disabled={busy||!attempt.saved}>{busy?'正在儲存……':'確認符合程度'}<ArrowRight size={17}/></button>{!attempt.saved&&<p className="chart-note">完成答案儲存後即可提交回饋，請使用上方「重新儲存」。</p>}</form>{detailsOpen&&<form className="feedback-details" onSubmit={e=>{e.preventDefault();void saveFeedback(true)}}><p>若你願意，再留下一點更具體的感受。</p><label>看完報告後，你現在最認同的文明 <span>（選填）</span><select value={self} onChange={e=>setSelf(e.target.value)}>{options}</select></label><label>最符合／最不像自己的地方 <span>（選填）</span><textarea value={comment} onChange={e=>setComment(e.target.value)} maxLength={2000} rows={4} placeholder="說說讓你有感的片段。請勿填寫姓名、聯絡方式等個人資料。"/></label><small>{comment.length} / 2000</small><button className="button" type="submit" disabled={busy}>{busy?'正在儲存……':'送出補充回饋'}<ArrowRight size={17}/></button></form>}<p className="form-status" role="status">{status}</p></div></section>;}

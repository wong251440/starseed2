...
const REPTILIAN_NODES:Node[]=[
 {code:'RE',label:'18. 爬蟲族',x:270,y:260},
 {code:'DR',label:'17. 天龍座人',x:700,y:260},
];
...
export default function CivilizationLineageTree({resolution}:{resolution:LineageResolution}){
 ...
 const nodes=kind==='lyran'?LYRAN_NODES:REPTILIAN_NODES;
 const connectors=kind==='lyran'?LYRAN_CONNECTORS:REPTILIAN_CONNECTORS;
  ...
  <div ref={scrollRef} className="lineage-tree-scroll" tabIndex={0} aria-label="文明親緣樹，可左右滑動查看完整圖表">
   <div className={`lineage-decision-canvas is-${kind}`}>
    <svg className="lineage-connectors" viewBox={`0 0 ${WIDTH} ${HEIGHT}`} aria-hidden="true">
     <defs>
      <marker id="decision-arrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="5" markerHeight="5" orient="auto"><path d="M0 0 8 4 0 8z" fill="#6f8098"/></marker>
      <marker id="decision-active-arrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="5" markerHeight="5" orient="auto"><path d="M0 0 8 4 0 8z" fill="#e7c65c"/></marker>
     </defs>
     {[...connectors,...optional].map(link=>{
      const isActive=activeEdges.has(`${link.from}>${link.to}`);
      return <polyline key={`${link.from}-${link.to}`} points={link.points} markerEnd={`url(#decision-${isActive?'active-':''}arrow)`} className={`decision-connector${isActive?' is-active':''}`}/>;
     })}
    </svg>
    {nodes.map(node=><IconNode key={node.code} node={node} active={active} final={resolution.lineageResult}/>)}
   </div>
  </div>
 </section>;
}

import {MODEL_VERSION,SCHEMA_VERSION,responseMode,validateResponses,isDraftAnswer,hasPresentationFlips,type Responses,type DraftResponses,type QuizMode,questionsForMode,wordingVersionsForMode} from './questionnaire';
...
export type Draft={modelVersion:string;mode:QuizMode;responses:DraftResponses;index:number;startedAt:string;referralCode:string|null;itemVersions:Record<string,number>;presentationFlips:Record<string,boolean>};
export type Attempt={mode:QuizMode;attemptId:string;participantId:string;feedbackToken:string;schemaVersion:number;modelVersion:string;responses:Responses;startedAt:string;completedAt:string;durationMs:number;imported:boolean;referralCode:string|null;itemVersions?:Record<string,number>;presentationFlips?:Record<string,boolean>;pretest?:Pretest;reportIntroSeen?:boolean;saved:boolean;result?:ScoringResult};
...
export function freshPresentationFlips(mode:QuizMode){const values=new Uint8Array(questionsForMode(mode).length);crypto.getRandomValues(values);return Object.fromEntries(questionsForMode(mode).map((question,index)=>[question.id,Boolean(values[index]&1)]));}
function neutralPresentationFlips(mode:QuizMode){return Object.fromEntries(questionsForMode(mode).map(question=>[question.id,false]));}
export function freshDraft(mode:QuizMode='full'):Draft{return {modelVersion:MODEL_VERSION,mode,responses:{},index:0,startedAt:new Date().toISOString(),referralCode:referralCode(),itemVersions:wordingVersionsForMode(mode),presentationFlips:freshPresentationFlips(mode)};}
export function loadDraft():Draft{
 ...
 if(d&&d.modelVersion===MODEL_VERSION&&d.responses&&typeof d.responses==='object'&&!Array.isArray(d.responses)){
  ...
  for(const mode of modes){if(Object.entries(d.responses).every(([id,value])=>{const q=questionsForMode(mode).find(q=>q.id===id);return q&&isDraftAnswer(q,value);})&&Number.isInteger(d.index)&&d.index>=0&&d.index<questionsForMode(mode).length&&Number.isFinite(Date.parse(d.startedAt))){const itemVersions=wordingVersionsForMode(mode),responses={...d.responses},presentationFlips=hasPresentationFlips(d.presentationFlips,mode)?d.presentationFlips:neutralPresentationFlips(mode);for(const question of questionsForMode(mode))if((d.itemVersions?.[question.id]??1)!==itemVersions[question.id])delete responses[question.id];return {...d,mode,responses,itemVersions,presentationFlips,referralCode:typeof d.referralCode==='string'||d.referralCode===null?d.referralCode:referralCode()};}}
 }
 return freshDraft();
}

...
export type Attempt={mode:QuizMode;attemptId:string;participantId:string;feedbackToken:string;schemaVersion:number;modelVersion:string;responses:Responses;startedAt:string;completedAt:string;durationMs:number;imported:boolean;appVersion:string;referralCode:string|null;saved:boolean;result?:ScoringResult};
...
export function newAttempt(responses:Responses,startedAt:string,imported:boolean,mode:QuizMode='full'):Attempt{validateResponses(responses,mode);const completedAt=new Date().toISOString();return {mode,attemptId:crypto.randomUUID(),participantId:participant(),feedbackToken:crypto.randomUUID(),schemaVersion:SCHEMA_VERSION,modelVersion:MODEL_VERSION,responses:structuredClone(responses),startedAt,completedAt,durationMs:Math.max(0,Date.parse(completedAt)-Date.parse(startedAt)),imported,appVersion:APP_VERSION,referralCode:referralCode(),saved:false};}

import {MODEL_VERSION,SCHEMA_VERSION,responseMode,validateResponses,isDraftAnswer,type Responses,type DraftResponses,type QuizMode,questionsForMode} from './questionnaire';
...
export type Draft={modelVersion:string;mode:QuizMode;responses:DraftResponses;index:number;startedAt:string;referralCode:string|null};
export type Attempt={mode:QuizMode;attemptId:string;participantId:string;feedbackToken:string;schemaVersion:number;modelVersion:string;responses:Responses;startedAt:string;completedAt:string;durationMs:number;imported:boolean;referralCode:string|null;pretest?:Pretest;saved:boolean;result?:ScoringResult};
...
export function freshDraft(mode:QuizMode='full'):Draft{return {modelVersion:MODEL_VERSION,mode,responses:{},index:0,startedAt:new Date().toISOString(),referralCode:referralCode()};}
...
export function newAttempt(responses:Responses,startedAt:string,imported:boolean,mode:QuizMode='full',lockedReferralCode:string|null=referralCode()):Attempt{validateResponses(responses,mode);const completedAt=new Date().toISOString();return {mode,attemptId:crypto.randomUUID(),participantId:participant(),feedbackToken:crypto.randomUUID(),schemaVersion:SCHEMA_VERSION,modelVersion:MODEL_VERSION,responses:structuredClone(responses),startedAt,completedAt,durationMs:Math.max(0,Date.parse(completedAt)-Date.parse(startedAt)),imported,referralCode:lockedReferralCode,saved:false};}

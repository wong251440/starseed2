...
import {resolveLineage,type LineageResolution} from './lineage-locator';
...
export interface ClassifiedPublicResult {status:'classified';primary:ResultLineage;runner_up:ResultLineage;scores:Record<string,number>;stability:PublicStability;classificationClarity:ClassificationClarity;lineage:LineageResolution}
export interface ClassifiedDiagnosticResult {status:'classified';model_version:string;ranking:RankingEntry[];raw:ClassifiedPRCS;lineage:LineageResolution}
...
export function adaptResult(raw:PRCSResult,mode:QuizMode='full'):ScoringResult{
 ...
 const lineageResolution=resolveLineage(raw.ranking,mode);
 const rawAnchor=lineage(raw.primary),primary=lineage(lineageResolution.lineageResult),runnerUp=lineage(raw.runner_up),classificationClarity=classifyClarity({form:mode,primary:rawAnchor.id,runnerUp:runnerUp.id,marginRaw:raw.primary_similarity-raw.runner_up_similarity,prototypeStable:raw.prototype_robustness.scenarios.every(s=>s.primary===raw.primary)});
 return {public:{status:'classified',primary,runner_up:runnerUp,scores,stability:{label:stable?'stable':'sensitive',all_variants_preserve_primary:stable},classificationClarity,lineage:lineageResolution},diagnostic:{status:'classified',model_version:raw.model_version,ranking,raw,lineage:lineageResolution}};
}

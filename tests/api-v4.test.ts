...
describe('RPCS API and historical migration',()=>{
 ...
 it('scores without collecting sample/shared answers',async()=>{
  ...
  expect(data.modelVersion).toBe(MODEL_VERSION);expect(data.result.public.primary.id).toBe('PO');expect(data.result.diagnostic.ranking).toHaveLength(21);expect(db.prepare('SELECT count(*) n FROM attempts').get()?.n).toBe(0);
 });
 it('recomputes results, saves new data idempotently and rejects changed answers for the same attempt',async()=>{
  ...
  expect(response.status).toBe(201);const data=await response.json() as any;expect(data.result.public.primary.id).toBe('PO');
  const stored=db.prepare('SELECT * FROM attempts').get() as any;expect(stored.primary_id).toBe(10);expect(stored.model_version).toBe(MODEL_VERSION);expect(JSON.parse(stored.raw_answers)).toEqual(demo);expect(JSON.parse(stored.metrics).robustness.classificationStabilityIndex).toBeCloseTo(data.result.public.stability.index,10);
  expect((await worker.fetch(request('/api/attempts',a),e)).status).toBe(200);
  expect((await worker.fetch(request('/api/attempts',{...a,responses:{...a.responses,'A-BP01':2}}),e)).status).toBe(409);

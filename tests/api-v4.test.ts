...
describe('PRCS API and historical migration',()=>{
 ...
 it('returns authoritative raw JSON for answers/direct bodies without collecting data',async()=>{
  ...
  const partial=await worker.fetch(request('/api/score',{answers:{'A-05':null,'D-08':4}}),e);expect(partial.status).toBe(200);const partialResult=await partial.json() as any;expect(partialResult.status).toBe('SENSITIVE');expect(partialResult.calibration_version).toBe('PRCS-EMP-v0.5-N518-20260919');

...
describe('PRCS API and historical migration',()=>{
 ...
 it('returns authoritative raw JSON for answers/direct bodies without collecting data',async()=>{
  ...
  for(const body of [{answers:demo.responses},demo.responses]){
   ...
   const d=await response.json() as any;expect(d.model_version).toBe('PRCS-v2.0');expect(d.selection_version).toBe('V4-RPCB-60');expect(d.model_fingerprint).toBe('79dec38680e6f71004b88d0d2c4e8162622251277ae3f9b5a9bfaeb036ab2396');expect(d.primary).toBe('PL');expect(d.item_dropout_robustness.by_k['3'].scenario_count).toBe(34220);
  }
